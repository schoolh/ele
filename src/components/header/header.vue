...
export default {
  ...
  data() {
    return {
      // 如果要改变data.json中以下几个的顺序，测试图标会不会跟着变，需要再npm run dev才会有变化
...
.header {
  ...
  .detail {
    ...
    height: 100vh;//vw是css3新增的，视窗高度
    //注意此处不能用min-height: 100vh，因为如果加了min- 该元素的长度可以一直增加，
    //不管里面多少内容，都不会溢出，就不会出现滚动条。
    overflow: auto;
